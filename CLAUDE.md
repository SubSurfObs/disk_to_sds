...
## Duplication diagnostics — `plotting/`

Stdlib + matplotlib scripts (from a parallel session) for investigating the
upstream-seedlink **duplicated/interleaved miniSEED** issue. See
`plotting/README.md` for full detail. Core idea: the **duration ratio** =
(sum of in-record sample-time) / 86400 — clean day ≈ 1.0, doubled day ≈ 2.0,
partial < 1.0. This is the *same metric* `apply.py` uses (its "density" check)
to detect duplication-corrupted LT days; the plotting tools are the analysis
side, `apply.py` is the operational side.

- **`scan_sds_durations.py <SDS_ROOT> <NET> <STA> <CHA> [csv]`** — core tool;
  walks an SDS tree, computes per-day ratio from record headers (stdlib only,
  assumes 512-byte records), writes CSV. Run on the VM against
  `/mnt/seiscomp_archive` or on a workstation against an SMB-mounted SD SDS.
- **`find_gaps.py <file> [min_gap_s]`** — per-record gap report for one file
  (why a ratio is < 1.0).
- **`check_stream_duration.py <file> <req_s>`** — quick verdict on a single
  fdsnws response.
- **`plot_*`** — matplotlib dashboards (read CSVs from `~/Downloads`, write
  PNGs there). `plot_duration_ratios.py` has hardcoded TRPU CSV paths;
  `plot_vw_file_sizes.py` needs a file-size CSV generated by a shell one-liner
  (in the README). File-size plots are kept for reference but duration ratio
  is the cleaner signal (file size has ±30-50% compression noise — why
  size-based completeness checks were rejected).

**Key finding the tools established:** SD-card data caps at ratio 1.0 (physics —
a recorder can't write >1 day/day), so it's ground truth. Same station via the
SeisComp pipeline shows ratios ~2.0 in discrete intervals (e.g. TRPU CHZ: 53
days at 1.99 across 2026-02-26→03-18 and 2026-04-10 on). Same station, two
paths, only the pipeline produces >1.0 — that contrast is the case made to the
upstream operator. Dependencies: diagnostic core is stdlib; plots need
matplotlib; nothing needs ObsPy or SeisComp.
